...
*/
{
    //there needs to be a function thing for opening the file in order to read it 
    while (true)
    {
        ...
        if (task.get_count() > 0)
        {
            task.display();
            std::cout << std::endl;
            std::cout << "Below are the features currently supported by the program: \n 1)  
            ...
            if (choice == "1" || choice == "1) Add Task" || choice == "Add Task" || choice == "1)")
			{
                ...
                task.remove();
			} else if (choice == "3" || choice == "3) View Tasks" || choice == "View Tasks" || choice == "3)")) {
